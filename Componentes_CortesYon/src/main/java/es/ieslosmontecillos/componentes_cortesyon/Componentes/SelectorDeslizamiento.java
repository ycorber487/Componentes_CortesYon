...
public class SelectorDeslizamiento extends AnchorPane {
    ...
    public SelectorDeslizamiento() {
        FXMLLoader fxmlLoader = new FXMLLoader(
                getClass().getResource("/es/ieslosmontecillos/componentes_cortesyon/selector_deslizamiento.fxml"));
    ...
    private void updateItem() {
        if (items.isEmpty()) {
            ...
            if (selectedIndex < 0) {
                selectedIndex = repetitive ? items.size() - 1 : 0;
            } else if (selectedIndex >= items.size()) {
                selectedIndex = repetitive ? 0 : items.size() - 1;
            }
